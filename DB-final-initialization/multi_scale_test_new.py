...
- 1,000 registros (1K) - 10 iteraciones
- 10,000 registros (10K) - 5 iteraciones  
- 100,000 registros (100K) - 3 iteraciones
- 1,000,000 registros (1M) - 1 iteración
...
class MultiScalePerformanceTester:
    def __init__(self):
        ...
        # Número de iteraciones por escala (optimizado por tiempo)
        self.iterations = {
            1000: 10,      # 1K: 10 iteraciones
            10000: 5,      # 10K: 5 iteraciones  
            100000: 3,     # 100K: 3 iteraciones
            1000000: 1     # 1M: 1 iteración
        }
        self.results = {}
    ...
    def run_performance_test(self, scale):
        ...
        # Ejecutar con el número específico de iteraciones
        command = f"python measure_performance.py --iterations {iterations}"
        success, output = self.run_command(command, f"Pruebas de rendimiento {scale_name}")
        ...
        if success:
            ...
            try:
                ...
                if os.path.exists(json_file):
                    ...
                    print(f"✅ Resultados {scale_name} guardados ({iterations} iteraciones)")
                    return True
                else:
    ...
    def run_full_multi_scale_test(self):
        ...
        print("📊 Escalas a probar:")
        for scale, scale_name in zip(self.scales, self.scale_names):
            iterations = self.iterations[scale]
            print(f"  • {scale_name}: {scale:,} registros ({iterations} iteraciones)")
    ...
    def generate_comparative_report(self):
        ...
        summary = {
            ...
            'iterations_config': self.iterations,
            'summary': {},
    ...
    def generate_comparative_latex(self):
        ...
        latex_content = f"""% Reporte Comparativo Multi-Escala - {datetime.now().strftime('%Y-%m-%d %H:%M')}
...
"""
        ...
        # Agregar filas de datos sin índices
...
\\end{{tabular}}
\\caption{{Tiempos de ejecución sin índices (ms) - Promedio de iteraciones variables por escala}}
\\label{{table:sin_indices_multi_scale}}
\\end{{table}}
...
"""
        ...
        # Agregar filas de datos con índices
        for i in range(1, 5):  # 4 consultas
            query_id = f"consulta_{i}"
            row = [f"Consulta {i}"]
        ...
        latex_content += f"""\\hline
\\end{{tabular}}
\\caption{{Tiempos de ejecución con índices (ms) - Promedio de iteraciones variables por escala}}
\\label{{table:con_indices_multi_scale}}
\\end{{table}}

% Tabla comparativa: Mejoras de rendimiento
\\begin{{table}}[h!]
\\centering
\\begin{{tabular}}{{|l|{'c|' * len(self.results)}}}
\\hline
...
"""
        ...
        latex_content += f"""\\hline
\\end{{tabular}}
\\caption{{Mejoras de rendimiento con índices (\\%) por escala de datos}}
\\label{{table:mejoras_multi_scale}}
\\end{{table}}

\\subsection{{Configuración de Iteraciones por Escala}}

Las pruebas se ejecutaron con diferentes números de iteraciones optimizadas por escala:
\\begin{{itemize}}
    \\item \\textbf{{1K registros}}: 10 iteraciones para máxima precisión estadística
    \\item \\textbf{{10K registros}}: 5 iteraciones para balance tiempo/precisión  
    \\item \\textbf{{100K registros}}: 3 iteraciones para reducir tiempo de ejecución
    \\item \\textbf{{1M registros}}: 1 iteración para factibilidad en tiempo razonable
\\end{{itemize}}

\\subsection{{Observaciones}}

Los resultados muestran el comportamiento escalable de los índices, donde las mejoras de rendimiento se vuelven más significativas con volúmenes de datos mayores.
"""
        ...
        print("✅ Archivo LaTeX comparativo generado: multi_scale_comparative.tex")
    ...
    def print_console_summary(self):
        ...
        for scale_name in self.results.keys():
            if 'total_records' in self.results[scale_name]:
                ...
                scale_index = self.scale_names.index(scale_name)
                scale_value = self.scales[scale_index]
                iterations = self.iterations[scale_value]
                
                print(f"\n📈 {scale_name} ({total_records:,} registros, {iterations} iteraciones):")
